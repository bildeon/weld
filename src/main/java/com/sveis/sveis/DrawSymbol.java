...
public class DrawSymbol extends JFrame implements ActionListener{  
	
	ArrayList<WeldPoint> sveiseListe = new ArrayList<>();
	List<WeldPoint> resultListe = new ArrayList<>();
	List<WeldPoint> resultatListe = new ArrayList<>();
    JLabel over;  
    JLabel aborter;
    JButton butt;
    JLabel a;
...
DrawSymbol(Point point ){  
    super("New image on weldingstring");  
   ...
   System.out.println( point);
    ...
    if ( antall == 0 ) {
    	aborter = new JLabel("No string generated");
  ...
  public   List<WeldPoint> getListe() {	  
	List<WeldPoint> resultatListe = new ArrayList<>(); 
	resultatListe = resultListe;
	return resultListe;
  }
...
//		System.out.println( vString + "  "  + resultListe.size());
	}
